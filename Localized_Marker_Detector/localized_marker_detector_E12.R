...
ad <- read_h5ad('anndata_objects/interactive/mesen_E12.h5ad')
...
DimPlot(object = mesen.seurat, reduction = "umap",group.by='annotation_all')
FeaturePlot(object = mesen.seurat, features = "Casz1")
...
cell_label = mesen.seurat$annotation_all

Gene_detected_count <- apply(dat > apply(dat,2,median),1,sum)
selected_genes = (Gene_detected_count >= 50) & (Gene_detected_count <= ncol(dat) * 0.8)
dat = dat[selected_genes,,drop = FALSE]

# Construct knn graph
knn_result = ConstructKnnGraph(knn = 5, feature_space = feature_space)
saveRDS(knn_result, 'r_env/knn_result_E12.rds')
...
gene_cl_res = ClusterGenes(dist, accu=.65,min_gene = 10,clustering_method = "complete", return_tree = TRUE, deepSplit = 1)
...
pl = FeaturePlot(mesen.seurat, features = names(gene_partition)[gene_partition == module_id][1:9],ncol = 3, order = TRUE) & NoAxes()
...
write.csv(res$gene_table, 'r_lmd_result_E12.txt')
