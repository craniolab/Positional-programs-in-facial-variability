...
ad <- read_h5ad('anndata_objects/interactive/mesen_E10_new.h5ad')
cell_cycle_genes = read.delim("cell_cycle_genes_mesen.txt",header = FALSE)
cell_cycle_genes = cell_cycle_genes$V1

kaucka_genes = read.delim("kaucka_genes_E10.txt",header = FALSE)
kaucka_genes = kaucka_genes$V1

equal_genes = read.delim("equal_genes_E10.txt",header=FALSE)
equal_genes = equal_genes$V1
...
mesen.seurat = subset(mesen.seurat, features = rownames(mesen.seurat)[rownames(mesen.seurat) %in% equal_genes]) #keep good genes
#mesen.seurat = subset(mesen.seurat, features = rownames(mesen.seurat)[!rownames(mesen.seurat) %in% kaucka_genes]) #remove kaucka genes

mesen.seurat <- NormalizeData(mesen.seurat, normalization.method = "LogNormalize", scale.factor = 10000)

DimPlot(object = mesen.seurat, reduction = "umap",group.by='trajectory_early')
DimPlot(object = mesen.seurat, reduction = "umap",group.by='source')
FeaturePlot(object = mesen.seurat, features = "Sox9")
#FeaturePlot(object = mesen.seurat, features = "mt-Atp8")
...
cell_label = mesen.seurat$trajectory_early

Gene_detected_count <- apply(dat > apply(dat,2,median),1,sum)
selected_genes = (Gene_detected_count >= 30) & (Gene_detected_count <= ncol(dat) * 0.5)
dat = dat[selected_genes,,drop = FALSE]

# Construct knn graph
knn_result = ConstructKnnGraph(knn = 5, feature_space = feature_space)
saveRDS(knn_result, 'r_env/knn_result_E10.rds')
...
#top_lmd_genes = rownames(res.cutoff[res.cutoff$score<4.2,])
...
gene_cl_res = ClusterGenes(dist, accu=.6,min_gene = 10,clustering_method = "complete", return_tree = TRUE, deepSplit = 1)
...
module_id = 3 #2 21 25 27-36 check 34
...
write.csv(res$gene_table, 'r_lmd_result_E10.txt')
